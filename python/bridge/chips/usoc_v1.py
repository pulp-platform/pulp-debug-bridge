...
class usoc_v1_debug_bridge(debug_bridge):

    def __init__(self, config, binaries=[], verbose=False):

        super(usoc_v1_debug_bridge, self).__init__(config=config, binaries=binaries, verbose=verbose)
    ...
    def stop(self):
        ...
        while True:
            # on usoc_v1, the core will start only after a while when the reset
